		...
		<NavigationContainer>
            {navOptions.map((option) => 
                <Option key={option.displayTitle}>
                    <TopLevelOption tabIndex={1} active={option.active}>
                        {option.displayTitle}
                        {option.children && <Carat active={option.active} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 76.6 139.5"><title>Decorative Icon</title><path d="M39.9,69.2,2.6,23.1C-1.2,18.5-.9,15.8,3.7,12L16.1,2C19.7-.9,23-.6,26,3,41.8,22.6,57.7,42.2,73.5,61.9c4.2,5.2,4.1,8.6.1,13.9L28.4,135.9c-3.3,4.2-6.2,4.7-10.6,1.5L5.1,127.8c-4-3-4.4-6.2-1.3-10.2C15.4,102,27.1,86.5,38.7,71,39.1,70.4,39.5,69.9,39.9,69.2Z"/></Carat>}
                    </TopLevelOption>
                    {option.children?.map((subOption) => 
                        <SubLevelOption tabIndex={1} parentActive={option.active} key={subOption.displayTitle} active={subOption.active}>{subOption.displayTitle}</SubLevelOption>
                    )}
                </Option>
            )}
		</NavigationContainer>
